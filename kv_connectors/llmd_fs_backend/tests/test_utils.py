...
# Pure-HMA models: hybrid architectures (Mamba/SSM, Qwen3.5 hybrid) whose
# "block" is not a fixed number of tokens. The connector must use its own
# internal block sizing, and we cannot pass `block_size=` to LLM().
PURE_HMA_MODELS = {
    "Qwen/Qwen3.5-27B",
...
# Interleaved-HMA models: standard 16-token blocks but mix sliding-window
# and full-attention layers, so the hybrid KV cache manager still applies.
# block_size tuning remains meaningful.
INTERLEAVED_HMA_MODELS = {
    "openai/gpt-oss-20b",
    "openai/gpt-oss-120b",
}


def resolve_hma(model_name, hma_override=None):
    """Classify a model for HMA (hybrid KV cache manager) handling.

    Returns (enable_hma, is_pure_hma):
      - enable_hma: pass disable_hybrid_kv_cache_manager=False to vLLM.
      - is_pure_hma: model has no fixed token-block size; the caller must
        drop block_size overrides.

    hma_override (True/False) forces enable_hma; None means auto-detect.
    is_pure_hma always reflects the model architecture.
    """
    is_pure_hma = model_name in PURE_HMA_MODELS
    auto_hma = is_pure_hma or model_name in INTERLEAVED_HMA_MODELS
    enable_hma = auto_hma if hma_override is None else hma_override
    return enable_hma, is_pure_hma
...
def get_test_configs(selected_test="all", block_size=16, num_cpu_blocks=50000):
    ...
    shared_storage_path=f"/ibm/fs1/kfir/native-fs/{random_id}"
    configs = [
        {
