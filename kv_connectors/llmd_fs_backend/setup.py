...
# (cp38-abi3 / Py_LIMITED_API isn't viable here: torch's bundled pybind11
# uses non-stable-API macros like PyTuple_SET_ITEM, so the moment we set
# the macro, every torch/extension.h consumer fails to compile. Aligning
# with vLLM's stable-ABI build would require dropping torch::cpp_extension
# and using a CMake build with a non-torch pybind11.)
