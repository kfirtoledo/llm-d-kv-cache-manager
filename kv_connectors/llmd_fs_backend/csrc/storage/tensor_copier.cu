...
void TensorCopier::copy_blocks_via_batch_memcpy(
  ...
  // CPU buffer layout for one file (back-of-buffer for partial last files):
  // [block0_layer0, block0_layer1, ..., blockK_layerN]
...
// Dispatches to one of three paths (batch (default), kernel, memcpy):
...
void TensorCopier::copy_blocks(uint8_t* cpu_base,
  ...
  if (use_batch) {
    copy_blocks_via_batch_memcpy(cpu_base, block_ids_list, group_idx, is_store);
  } else if (use_kernel) {
