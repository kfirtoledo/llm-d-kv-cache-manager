...
def run_generation_test(name: str,
    ...
    for i in range(num_req):
        ...
        if test_dir:
            ...
            print(f"[{i+1}] {dt:.3f}s | {conc_part}{get_line} | {put_line} | {lk_line} | Δ{d_files:+d} files ({d_bytes / (1 << 20):+.1f} MB) → {len(new_sizes)} files / {sum(new_sizes) / (1 << 20):.1f} MB (avg {avg_mb:.2f} MB/file)")
            prev_sizes = new_sizes
        else:
            print(f"[{i+1}] {dt:.3f}s | {conc_part}{get_line} | {put_line} | {lk_line} | {text[:80].replace(chr(10),' ')}")
